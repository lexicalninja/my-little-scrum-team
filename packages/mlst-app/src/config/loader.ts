...
import { MLSTConfigSchema, type MLSTConfig } from './schema.js';
...
export async function loadConfig(sources: ConfigSources = {}): Promise<MLSTConfig> {
  const { cliFlags = {}, projectDir = process.cwd() } = sources;

  // 1. Global config (~/.mlst/config.json)
  const globalConfig = await readJsonFile(join(homedir(), '.mlst', 'config.json'));

  // 2. Project config (.mlst/config.json)
  const projectConfig = await readJsonFile(resolve(projectDir, '.mlst', 'config.json'));

  // 3. Env-specified config file
  const envConfigPath = process.env.MLST_CONFIG_PATH;
  ...
  return MLSTConfigSchema.parse(merged);
}
