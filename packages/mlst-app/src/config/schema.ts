...
export const MLSTConfigSchema = z.object({
  ...
  stateDir: z.string().default('.mlst'),
...
export type MLSTConfig = z.infer<typeof MLSTConfigSchema>;

export function getDefaultConfig(): MLSTConfig {
  return MLSTConfigSchema.parse({});
}

export function mergeWithDefaults(partial: Record<string, unknown>): MLSTConfig {
  return MLSTConfigSchema.parse(partial);
}
