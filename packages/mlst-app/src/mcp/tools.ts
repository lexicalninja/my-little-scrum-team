...
export function createMCPTools(engine: OrchestrationEngine, config: MLSTConfig): MCPToolDefinition[] {
  return [
    {
      name: 'mlst_build',
      description: 'Start a new build. Orchestrates specialist AI agents to implement software from a description. If the response contains `needs_clarification: true`, ask the user the `question` field, then call `mlst_resume` with `runId` and `context` set to the user\'s answer.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'mlst_config',
      description: 'View current MLST configuration.',
      inputSchema: { type: 'object', properties: {} },
      handler: async () => {
