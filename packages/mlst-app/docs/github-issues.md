...
MLST can fetch a GitHub issue and use its title and body as the build description, so the AI agents receive meaningful context instead of a bare `#25` reference.

## Feature Overview

When you pass an issue reference to `mlst build`, the CLI:
...
| `mlst build 'owner/repo#25'` | Issue 25 in the named repo (no git remote lookup needed) |
| `mlst build 'implement the feature from #25'` | Inline reference — `#25` is expanded in place within the text |
...
mlst build 'owner/repo#25'

# Inline reference embedded in a description
mlst build 'implement the feature described in #25'
```
...
MLST checks authentication before every issue fetch. If the check fails you will see a clear error message (see [Error Reference](#error-reference)).
