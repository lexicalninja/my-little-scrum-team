 ...
 *   4. MCP mlst_build with fromIssue: engine.build called with resolved string
 *   5. MCP mlst_build without fromIssue: engine.build called with raw description
 */
...
// and mlst.ts are NOT individually mocked — they all run their real code.
...
// bin/mlst.ts imports (loadConfig, engine, interaction, provider …).
// Import bin/mlst.ts dynamically so Commander fires the action handler.
...
describe('Smoke 1 — CLI --from-issue path', () => {
  ...
  beforeEach(() => {
    ...
    // gh issue view.  (bin/mlst.ts calls inferRepoFromGitRemote first for
  ...
  it('passes a string starting with "GitHub Issue #" to engine.build when --from-issue is used', async () => {
    process.argv = ['node', 'mlst', 'build', '--from-issue', '7'];
...
describe('Smoke 2 — CLI free-text #N path', () => {
  ...
  it('injects resolved issue content into the description when #N is present in free-text', async () => {
    process.argv = ['node', 'mlst', 'build', 'implement', '#7'];
...
describe('Smoke 3 — CLI free-text no #N path', () => {
  ...
  it('passes the raw string to engine.build and never calls execFile when no #N is present', async () => {
    process.argv = ['node', 'mlst', 'build', 'add', 'user', 'authentication'];
...
// ─── Smoke Tests 4 & 5: MCP mlst_build handler ─────────────────────────────
//
...
describe('Smoke 4 — MCP mlst_build with fromIssue', () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it('engine.build is called with the resolved issue string when fromIssue is provided', async () => {
    ...
    const config = {
      ...
      stateDir: '/tmp/mlst-smoke',
      saveDecisions: false,
      decisionsDir: '/tmp/mlst-smoke/decisions',
    };
    ...
    const mlstBuild = tools.find((t) => t.name === 'mlst_build')!;

    await mlstBuild.handler({ description: '', fromIssue: 7 });
...
describe('Smoke 5 — MCP mlst_build without fromIssue', () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it('engine.build is called with the raw description and execFile is never called when fromIssue is absent', async () => {
    ...
    await mlstBuild.handler({ description: 'build the login page' });
