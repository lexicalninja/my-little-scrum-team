/**
 * Unit tests for src/mcp/tools.ts — mlst_build handler
 *
 * Acceptance criteria:
 * AC-1: mlst_build with fromIssue calls fetchGitHubIssue and passes resolved
 *       content to engine.build()
 * AC-2: mlst_build without fromIssue passes description directly to engine.build()
...
/** Minimal MLSTConfig — only the fields createMCPTools uses */
function makeConfig(): MLSTConfig {
  return {
    models: {},
    api: { baseURL: 'https://example.com' },
    maxSteps: 5,
    maxReviewIterations: 1,
    stateDir: '/tmp/mlst-test',
    saveDecisions: false,
    decisionsDir: '/tmp/mlst-test/decisions',
  };
}
...
describe('mlst_build handler', () => {
  ...
  describe('AC-1: fromIssue resolves via fetchGitHubIssue', () => {
    it('calls fetchGitHubIssue with the correct issue number when fromIssue is provided', async () => {
      ...
      await mlstBuild.handler({ description: '', fromIssue: 25 });
    ...
    it('passes the resolved issue content (not the original description) to engine.build()', async () => {
      ...
      await mlstBuild.handler({ description: 'original description', fromIssue: 25 });
  ...
  describe('AC-2: without fromIssue, description passes through unchanged', () => {
    it('passes description directly to engine.build() when fromIssue is absent', async () => {
      ...
      await mlstBuild.handler({ description: 'add user authentication' });
    ...
    it('does not call fetchGitHubIssue when fromIssue is absent', async () => {
      ...
      await mlstBuild.handler({ description: 'build the thing' });
  ...
  describe('AC-3: no console.log is called during fromIssue resolution', () => {
    ...
    it('does not call console.log when fromIssue is absent', async () => {
      ...
      await mlstBuild.handler({ description: 'build auth module' });
  ...
  describe('AC-4: errors propagate as thrown Error, not process.exit', () => {
    it('throws an Error when fetchGitHubIssue rejects', async () => {
      ...
      await expect(
        mlstBuild.handler({ description: '', fromIssue: 25 }),
