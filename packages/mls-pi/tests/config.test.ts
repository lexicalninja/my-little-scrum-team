...
import {
  ...
  mergeConfigs,
  type ProjectConfig,
} from "../.pi/extensions/mls/config.js";

describe("loadConfigFile", () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mls-config-"));
  });
...
describe("mergeConfigs", () => {
  ...
  it("merges models.agents key-by-key", () => {
    const global: ProjectConfig = {
      models: {
        agents: { "mls-designer": "a/designer", "mls-spec-writer": "a/spec" },
      },
    };
    const project: ProjectConfig = {
      models: {
        agents: { "mls-designer": "b/designer", "mls-code-reviewer": "b/reviewer" },
      },
    };

    const result = mergeConfigs(global, project);
    expect(result.models?.agents).toEqual({
      "mls-designer": "b/designer",
      "mls-spec-writer": "a/spec",
      "mls-code-reviewer": "b/reviewer",
    });
  });
...
describe("loadProjectConfig", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mls-load-"));
  ...
  it("loads global config when only global exists", () => {
    ...
    const globalDir = path.join(fakeHome, ".mls");
  ...
  it("merges global and project configs with project taking precedence", () => {
    const cwd = path.join(tmpDir, "project");
    const projectMls = path.join(cwd, ".mls");
    const globalMls = path.join(fakeHome, ".mls");

    fs.mkdirSync(projectMls, { recursive: true });
    fs.mkdirSync(globalMls, { recursive: true });

    fs.writeFileSync(
      path.join(globalMls, "config.json"),
      JSON.stringify({
        mode: "fast",
    ...
    fs.writeFileSync(
      path.join(projectMls, "config.json"),
      JSON.stringify({
