...
const PrdSessionRunSpy = vi.fn().mockResolvedValue({ filePath: ".mls/prd-test.md", title: "Test" });
const PrdSessionResumeSpy = vi.fn().mockResolvedValue({ filePath: ".mls/prd-test.md", title: "Test" });
vi.mock("../.pi/extensions/mls/prd.js", () => ({
  ...
  parsePrdFilePath: (p: string) => /\.mls\/prd-[\w-]+\.md$/.test(p),
}));
...
/**
 * Register mlsExtension with a mock pi API and capture the 'prd' command handler.
 */
