...
describe("parsePrdFilePath", () => {
  it("recognizes a .mls/prd-*.md file path", () => {
    expect(parsePrdFilePath(".mls/prd-my-feature.md")).toBe(true);
  });

  it("recognizes a relative path with .mls/prd prefix", () => {
    expect(parsePrdFilePath(".mls/prd-sprint-dashboard.md")).toBe(true);
  });

  it("rejects non-PRD file paths", () => {
    expect(parsePrdFilePath("src/index.ts")).toBe(false);
    expect(parsePrdFilePath("README.md")).toBe(false);
    expect(parsePrdFilePath(".mls/other-file.md")).toBe(false);
  });
...
describe("PrdSession", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mls-prd-test-"));
  });
  ...
  it("runs a full session producing a PRD file", async () => {
    ...
    expect(result.filePath).toMatch(/\.mls\/prd-.*\.md$/);
  ...
  it("creates .mls directory if it does not exist", async () => {
    const deps = buildPrdDeps({
      cwd: tmpDir,
      llm: {
    ...
    expect(fs.existsSync(mlsDir)).toBe(true);
  });
  ...
  it("saves draft state to .mls/prd-<slug>.draft.json", async () => {
    const deps = buildPrdDeps({
      cwd: tmpDir,
      llm: {
    ...
    const mlsDir = path.join(tmpDir, ".mls");
    const draftFiles = fs.readdirSync(mlsDir).filter((f) => f.endsWith(".draft.json"));
    // Draft file is cleaned up after successful completion, so check the final PRD instead
    const prdFiles = fs.readdirSync(mlsDir).filter((f) => f.startsWith("prd-") && f.endsWith(".md"));
  ...
  it("resumes from an existing draft", async () => {
    ...
    fs.writeFileSync(
      path.join(mlsDir, "prd-resumed-feature.draft.json"),
...
describe("/prd command registration — source wiring", () => {
  it("index.ts registers a 'prd' command via pi.registerCommand", () => {
    const src = fs.readFileSync(
      path.resolve(__dirname, "..", ".pi", "extensions", "mls", "index.ts"),
      "utf-8",
    );
