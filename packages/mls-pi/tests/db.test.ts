...
import { MlsDatabase } from "../.pi/extensions/mls/db.js";

describe("MlsDatabase", () => {
  let tmpDir: string;
  let db: MlsDatabase;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mls-db-test-"));
    db = new MlsDatabase(tmpDir);
  });
  ...
  describe("Issue CRUD", () => {
    ...
    it("getIssue returns created issue", () => {
      ...
      expect(fetched!.assigned_agent).toBe("mls-developer");
    });
