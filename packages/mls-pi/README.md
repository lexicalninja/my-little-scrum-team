# My Little Scrum Team (MLS) — Pi Extension
...
- Register custom slash commands (e.g., `/build`, `/mls-status`)
...
- **`/mls-status`** — Shows the current sprint status from the local database.

One command kicks off the entire workflow. You describe what you want, and MLS coordinates 7 specialist agents through 5 phases to deliver it.
...
/build "add user authentication"
  ...
  [AGENT]  mls-spec-writer → detailed specification
  [LLM]   Gate 1: evaluate spec completeness

  Phase 2: Task Breakdown
  [AGENT]  mls-scrum-master → atomic task list (TASK-001, TASK-002, ...)
  ...
  [AGENT]  mls-impl-engineer → init project, install deps, wire DB,
  ...
  [AGENT]  mls-designer + mls-infra-engineer + mls-impl-engineer (docs)
           No TDD loop — produce artifacts and mark complete

  Group 2: Implementation (per task, up to 5 iterations)
  [AGENT]  mls-test-runner → write failing tests (RED)
  [AGENT]  mls-impl-engineer → make tests pass (GREEN)
  [CODE]   Run tests deterministically (exit code 0 = pass)
  [CODE]   Run linter deterministically
  [AGENT]  mls-code-reviewer → structured review
...
On greenfield projects (no source files detected), the orchestrator spawns `mls-impl-engineer` before any tasks to wire the tech stack end-to-end: initialize the project, install dependencies, connect the real database driver, create a health check endpoint, and run an integration test proving it all works. This prevents a common failure mode where per-task TDD creates in-memory fakes instead of using real dependencies.
...
| Agent | Role | Tools |
|-------|------|-------|
| `mls-spec-writer` | Transforms ideas into detailed specifications | read, grep, find, ls |
| `mls-scrum-master` | Breaks specs into atomic, dependency-ordered tasks | read, grep, find, ls |
| `mls-impl-engineer` | Implements code to make failing tests pass | read, edit, write, bash, grep, find, ls |
| `mls-test-runner` | Writes failing tests from acceptance criteria (TDD red phase) | read, write, bash, grep, find, ls |
| `mls-code-reviewer` | Reviews code against acceptance criteria — rejects only for bugs, security issues, or spec violations | read, grep, find, ls, bash |
| `mls-designer` | Creates UI/UX design specifications (layout, components, accessibility) | read, write, grep, find, ls |
| `mls-infra-engineer` | Sets up CI/CD, Docker, deployment configs | read, edit, write, bash, grep, find, ls |
...
MLS supports role-based model routing via `.mls/config.json` at two levels:

| Location | Scope |
|----------|-------|
| `~/.mls/config.json` | Global — applies to every project |
| `<project>/.mls/config.json` | Project-local — overrides global for that repo |
...
If you do nothing, MLS inherits the model from the parent pi session for all agents and direct LLM calls.

Resolution order for agent subprocesses:

1. Config exact agent override: `models.agents["mls-..."]`
...
This lets you set stable defaults once in `~/.mls/config.json` and override per-project only when needed.
...
Create `~/.mls/config.json` for global defaults that apply everywhere, and/or `<project>/.mls/config.json` for per-project overrides:

**Global** (`~/.mls/config.json`) — your baseline for all projects:
...
**Project** (`<project>/.mls/config.json`) — overrides only what differs:
...
The merged result keeps `default`, `planning`, `review`, `tests`, and `providers` from global, while `coding` and `agents.mls-designer` come from the project config.

This is useful when you want MLS-specific model defaults or when the built-in provider profile doesn't match your setup (e.g., you have a paid Google API key, not free-tier).
...
|-------|---------------|-----------|---------|
| `design-system` | mls-designer | `AGENT_SKILLS` in `types.ts` | Reference guide for layout, components, color, typography, spacing, accessibility |
...
Sprint state is persisted to a SQLite database at `<project-root>/.mls/mls.db`. Each project gets its own database. The schema tracks:
...
Add `.mls/` to your project's `.gitignore` — it contains local runtime state, not source.
...
Every `/build` invocation writes a JSONL log to `.mls/runs/<timestamp>.jsonl`. Each line is a JSON object with a `type` field matching the `MlsEvent` union in `types.ts`:

```
{"type":"sprint_start","input":"...","classification":"feature","timestamp":1774471423101}
{"type":"phase","phase":"phase1","timestamp":1774471423101}
{"type":"agent_start","agent":"mls-spec-writer","taskLabel":"","timestamp":1774471423200}
...
{"type":"agent_end","agent":"mls-spec-writer","usage":{...},"timestamp":1774471430000}
...
tail -f .mls/runs/*.jsonl | jq .

# Show all gate results
cat .mls/runs/*.jsonl | jq 'select(.type == "gate")'

# Show agent costs
cat .mls/runs/*.jsonl | jq 'select(.type == "agent_end") | {agent, cost: .usage.cost, model}'
```
...
├── .pi/extensions/mls/                  # Extension source (TypeScript)
│   ├── index.ts                         # Command registration (/build, /mls-status),
...
│   │                                    # MlsEvent union, AGENT_SKILLS mapping
│   ├── state.ts                         # StateManager — sprint state + UI sync
│   ├── db.ts                            # SQLite schema (SCHEMA constant), MlsDatabase class
...
5. Streams events back to the dashboard via `MlsEvent` emissions
...
This copies the extension to `~/.pi/agent/extensions/mls/` and installs runtime dependencies there.
...
```bash
ln -s /absolute/path/to/my-little-scrum-team-pi/.pi/extensions/mls ~/.pi/agent/extensions/mls
```
...
ln -s /absolute/path/to/my-little-scrum-team-pi/.pi/extensions/mls .pi/extensions/mls
```

### 3. Verify

Start pi in any project. You should see `/build` and `/mls-status` available as commands.
...
MLS works with whatever model provider you configure in pi. Pi supports subscription-based authentication, API keys, and cloud provider credentials.
...
|----------|----------|-------|
| **OpenRouter** | [openrouter.ai](https://openrouter.ai) | Routes to 100+ models from many providers. Free and paid tiers. MLS detects the sub-provider from the model ID (e.g., `google/gemini-3-flash-preview` applies Google rate profiles) |
...
All extension source lives in `.pi/extensions/mls/`. Edit these files directly — if you installed via `npm run dev` (symlink), changes take effect on the next pi session.
...
During development, the SQLite database is created at `<cwd>/.mls/mls.db`. You can inspect it directly:

```bash
sqlite3 .mls/mls.db ".tables"
sqlite3 .mls/mls.db "SELECT * FROM sprints ORDER BY created_at DESC LIMIT 5;"
```
