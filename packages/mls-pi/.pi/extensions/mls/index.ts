 ...
 *
 * Registers /build and /mls-status commands. Orchestrates 7 specialist
...
/** Mutable state backing the live MLS multi-agent widget in pi's TUI. */
...
const AGENT_STYLES: Record<string, { color: string; initial: string }> = {
  "mls-designer":       { color: "accent",  initial: "D" },
  "mls-spec-writer":    { color: "accent",  initial: "S" },
  "mls-impl-engineer":  { color: "success", initial: "I" },
  "mls-test-runner":    { color: "warning", initial: "T" },
  "mls-reviewer":       { color: "info",    initial: "R" },
  "mls-scrum-master":   { color: "muted",   initial: "M" },
};
...
/** Absolute path to the directory containing this module (`.pi/extensions/mls/`). */
const MODULE_DIR = path.dirname(fileURLToPath(import.meta.url));

/**
 * Resolve the plugin root directory (three levels up from the extension source).
 *
 * Layout: `<plugin-root>/.pi/extensions/mls/index.ts` → returns `<plugin-root>`.
 *
...
function resolveResourceDir(name: string): string {
  ...
  const candidates = [
    ...
    // packages/mls-pi → packages → repo root
    path.join(pluginRoot, "..", "..", name),
  ];
...
function updateWidget(widget: WidgetState): void {
  ...
  ctx.ui?.setWidget?.("mls-live", (_tui, theme) => createLiveWidget(widget, theme));
}

/** Format an agent name for display: strip `mls-` prefix, uppercase, replace hyphens. */
function shortAgent(name: string): string {
  return name.replace(/^mls-/, "").replace(/-/g, " ").toUpperCase();
}
  ...
  return {
    render(width: number) {
      ...
      if (widget.phase) {
        ...
        lines.push(`  ${prefix} ${bold(theme.fg("accent", "MLS Build"))}${counter}${concurrency}  ${theme.fg("dim", elapsed)}`);
        lines.push("");
      }
      ...
      if (lines.length === 0) {
        lines.push(theme.fg("dim", `  ${spinner} MLS idle`));
      }
...
function stopWidget(widget: WidgetState): void {
  ...
  const ctx = widget.ctx;
  if (ctx) {
    ctx.ui?.setWidget?.("mls-live", undefined);
  }
...
function applyFooterEvent(footer: FooterState, event: MlsEvent): void {
  ...
  ctx.ui?.setFooter?.((_tui, theme) => {
    ...
    return {
      ...
      render(width: number): string[] {
        ...
        lines.push(`${bold("mls")} ${theme.fg("dim", "│")} ${theme.fg("dim", elapsed)}${dashLink}`);
 ...
 * @param event  - An {@link MlsEvent} emitted by the orchestrator.
 */
function applyWidgetEvent(widget: WidgetState, event: MlsEvent): void {
  switch (event.type) {
    case "phase": {
...
export const __test__: {
  ...
  applyWidgetEvent(widget: any, event: MlsEvent): void;
  createFooterState(): any;
  applyFooterEvent(footer: any, event: MlsEvent): void;
...
function startDashboard(ctx: CommandContext): SessionHandle {
  ...
  ctx.ui?.setStatus?.("mls-dashboard", session.url);
  return session;
}
...
function stopDashboard(ctx: CommandContext, session: SessionHandle): void {
  ...
  session.stop();
  ctx.ui?.setStatus?.("mls-dashboard", undefined);
}
...
/**
 * Check whether the current process was launched as an MLS subprocess.
 *
...
/**
 * Format sprint status as an array of display lines for the `/mls-status` command.
 *
 ...
 * @param summary    - Aggregate counts from `MlsDatabase.getSprintSummary()`.
 * @param issues     - Issue rows from `MlsDatabase.getSprintIssues()`.
 ...
 * case so the MLS extension does not load inside its own subprocesses (re-entrancy guard).
 *
 * **Registrations:**
 * - `/build` command — starts the full orchestration pipeline.
 * - `/mls-status` command — shows current sprint status from SQLite.
 *
 * **Event hooks:**
 * - `session_start` — lazily initializes `StateManager` and `MlsDatabase` (singleton pattern;
 ...
 */
export default function mlsExtension(pi: ExtensionAPI): void {
  ...
  let db: MlsDatabase | null = null;
  let orchestratorActive = false;

  function ensureInit(ctx: CommandContext): { state: StateManager; db: MlsDatabase } {
    // Singleton pattern: state and db live for the entire pi session, not per-command.
    // Re-using the same instances preserves in-memory sprint state across /build and /mls-status.
    if (!state || !db) {
      state = new StateManager(createStateManagerDeps(ctx));
      db = new MlsDatabase(ctx.cwd);
    }
  ...
  async function handleResume(args: string, ctx: CommandContext, piApi: ExtensionAPI): Promise<void> {
    ...
    const tasks = issues.map((issue) => ({
      ...
      assignedAgent: issue.assigned_agent ?? "mls-impl-engineer",
      iterationCount: 0,
    }));
    ...
    try {
      ...
      ctx.ui?.notify?.("MLS resume complete.", "success");
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
      ctx.ui?.notify?.(`MLS resume failed: ${message}`, "error");
    } finally {
      orchestratorActive = false;
      stopWidget(widget);
      stopFooter(footer);
      ctx.ui?.setWorkingMessage?.();
      ctx.ui?.setStatus?.("mls", undefined);
  ...
  const handleBuild: CommandHandler = async (args, ctx) => {
    ...
    orchestratorActive = true;
    startWidget(widget, ctx);
    ctx.ui?.notify?.("MLS starting...", "info");
    ...
    try {
      ...
      ctx.ui?.notify?.("MLS workflow complete.", "success");
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
      ctx.ui?.notify?.(`MLS failed: ${message}`, "error");
    } finally {
      orchestratorActive = false;
      stopWidget(widget);
      stopFooter(footer);
  ...
  pi.on("tool_call", async (rawEvent) => {
    ...
    if (isBlockedTool(event.toolName)) {
      return {
        block: true,
        reason: `MLS: ${event.toolName} blocked during orchestration — only spawned agents may modify files.`,
      };
    }
    ...
    if (event.toolName === "bash") {
      ...
      if (blocked) {
        return { block: true, reason: `MLS safety: ${blocked}` };
      }
    }
  ...
  pi.registerCommand("build", {
    description: "Start the MLS scrum team workflow for any feature, epic, or bug fix",
    handler: handleBuild,
  });
  ...
  pi.registerCommand("mls-status", {
    description: "Show current MLS sprint status from database",
  ...
  pi.on("session_before_compact", async () => {
    ...
    return {
      customInstructions: `Preserve MLS sprint state. Phase: ${state.getState().phase}. Tasks: ${state.getState().tasks.length}.`,
    };
  });
}
