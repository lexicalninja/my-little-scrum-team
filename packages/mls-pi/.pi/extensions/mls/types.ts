/**
 * MLS Pi Extension — Shared Type Definitions
 */
 ...
 * - `phase1`         — Specification: `mls-spec-writer` agent drafts the full project spec.
 * - `phase2`         — Task breakdown: `mls-scrum-master` agent converts the spec to tasks.
 * - `scaffold`       — Scaffolding: `mls-impl-engineer` creates initial project structure for empty repos.
...
export interface SprintState {
  ...
  /** Full specification text produced by the `mls-spec-writer` agent in Phase 1. */
...
export interface MlsAgentConfig {
  name: string;
  description: string;
  tools?: string[];
  model?: string;
...
export const AGENT_SKILLS: Record<string, string[]> = {
  "mls-designer": ["design-system"],
};

// ─── Dashboard Events ───────────────────────────────────────────────────────

/** Common fields present on every dashboard event. */
interface MlsEventBase {
  timestamp: number;
...
export type MlsEvent =
  | (MlsEventBase & { type: "sprint_start"; input: string; classification: string })
  | (MlsEventBase & { type: "phase"; phase: string })
  | (MlsEventBase & {
      type: "human_gate";
      gate: GatePoint;
  ...
  | (MlsEventBase & { type: "agent_start"; agent: string; prompt: string; taskLabel: string })
  | (MlsEventBase & { type: "agent_end"; agent: string; output: string; model?: string; usage: UsageStats; taskLabel: string })
  | (MlsEventBase & { type: "agent_progress"; agent: string; taskLabel: string; text: string; toolCount: number })
  | (MlsEventBase & { type: "llm_start"; purpose: string; system: string; user: string; tier: string })
  | (MlsEventBase & { type: "llm_end"; purpose: string; response: string })
  | (MlsEventBase & { type: "exec_start"; command: string; args: string[] })
  | (MlsEventBase & { type: "exec_end"; command: string; code: number; stdout: string })
  | (MlsEventBase & { type: "gate"; name: string; passed: boolean; issues: string[] })
  | (MlsEventBase & { type: "task"; id: string; status: string; title: string })
  | (MlsEventBase & {
      type: "review";
      taskId: string;
      label: string;
  ...
  | (MlsEventBase & {
      ...
      taskId: string;
      /**
       * Git stash ref label used for this checkpoint (e.g., `"mls-checkpoint-<taskId>"`).
  ...
  | (MlsEventBase & { type: "sprint_end"; summary: string; aborted?: boolean; abortReason?: string });
