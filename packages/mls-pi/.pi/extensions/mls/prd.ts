/**
 * MLS Pi Extension — PRD (Product Requirements Document) Session
 *
...
export interface PrdResult {
  /** Relative path to the generated PRD file (e.g., `.mls/prd-my-feature.md`). */
  filePath: string;
 ...
 *
 * Matches paths of the form `.mls/prd-<slug>.md` (relative to project root).
...
export function parsePrdFilePath(filePath: string): boolean {
  ...
  return /(?:^|\/)\.mls\/prd-[\w-]+\.md$/.test(normalized);
}
 ...
 * // result.filePath → ".mls/prd-sprint-metrics-dashboard.md"
 * ```
 */
export class PrdSession {
   ...
   * 3. Save incremental state to `.mls/prd-<slug>.draft.json` after each answer.
   * 4. Build and write the final PRD to `.mls/prd-<slug>.md`.
   ...
   *
   * Reads `.mls/prd-<slug>.draft.json`, skips already-answered questions,
  ...
  /**
   * Build the PRD Markdown and write it to `.mls/prd-<slug>.md`.
   *
  ...
  private writePrd(state: PrdDraftState, slug: string): string {
    const mlsDir = path.join(this.deps.cwd, ".mls");
    fs.mkdirSync(mlsDir, { recursive: true });

    const relativePath = `.mls/prd-${slug}.md`;
   ...
   *
   * @param slug - File name slug.
   * @returns Absolute path to `.mls/prd-<slug>.draft.json`.
   */
  private draftPath(slug: string): string {
    return path.join(this.deps.cwd, ".mls", `prd-${slug}.draft.json`);
  }
}
