/**
 * MLS Pi Extension — Orchestrator (Blueprint Engine)
 *
...
import type { AgentResult, ClarificationRequest, Classification, DeletionCheckResult, EscalationAction, ExecutionProfile, GatePoint, HumanGateResult, MlsAgentConfig, MlsEvent, Phase, ReviewReason, TaskState, TaskType } from "../types.js";
import type { Issue, MlsDatabase } from "../db.js";
...
/**
 * Central coordinator for the MLS build pipeline.
 *
 ...
 * `StateManager` (in-memory + UI), `MlsDatabase` (SQLite), and the
...
export class Orchestrator {
  ...
  private getAgent(name: string): MlsAgentConfig {
...
Answer each question directly and briefly. Do not ask follow-up questions.`;
   ...
   * 3. SQLite via `MlsDatabase.updateIssue()` (persistent; survives interruptions).
   *
...
IMPORTANT: If the input is a document with sections like "Overview", "Tech Stack", "Acceptance Criteria", "Constraints" — it is "requirements", NOT "implementation-spec". An implementation-spec must have literal file paths and code diffs.`,
   ...
   * 1. `mls-impl-engineer` with bug-fix prompt (write failing test → fix → verify).
  ...
  private async fastPath(input: string): Promise<string> {
    ...
    let impl = await this.spawn("mls-impl-engineer", this.d.context.buildBugFixPrompt(input, orientation));
   ...
   * 1. `mls-impl-engineer` with impl-from-spec prompt.
   * 2. `ensureTestsPass` auto-fix loop.
   * 3. `runLint`.
   * 4. `reviewLoop`.
   *
  ...
  private async implFastPath(input: string): Promise<string> {
    ...
    const orientation = await this.getProjectOrientation();
    let impl = await this.spawn("mls-impl-engineer", this.d.context.buildImplFromSpecPrompt(input, orientation));
    impl = await this.ensureTestsPass(impl, input);
    await this.runLint();
   ...
   * 1. Specification — `mls-spec-writer` produces a full project spec.
   * 2. Task breakdown — `mls-scrum-master` converts spec to tasks; persisted to SQLite.
   ...
   *
   * Spawns `mls-spec-writer` with the spec prompt. If `enableSpecGate` is true,
  ...
  private async phase1(input: string): Promise<string> {
    ...
    const orientation = await this.getProjectOrientation();
    let spec = await this.spawn("mls-spec-writer", this.d.context.buildSpecPrompt(input, undefined, undefined, orientation));
   ...
   *
   * Spawns `mls-scrum-master` and passes the spec. Parses the agent output into
  ...
  private async phase2(spec: string): Promise<TaskState[]> {
    ...
    const taskOutput = await this.spawn("mls-scrum-master", this.d.context.buildTaskBreakdownPrompt(spec, annotations || undefined));
...
`;
  ...
  private async scaffold(spec: string): Promise<void> {
    ...
    await this.spawn("mls-impl-engineer", this.d.context.buildScaffoldPrompt(spec));
  ...
  private async executeGroup1(tasks: TaskState[], spec: string): Promise<void> {
    ...
    const jobs = [
      ...design.map((t) => ({ agent: this.getAgent("mls-designer"), task: this.d.context.buildDesignPrompt(t, spec) })),
      ...infra.map((t) => ({ agent: this.getAgent("mls-infra-engineer"), task: this.d.context.buildInfraPrompt(t, spec) })),
      ...docs.map((t) => ({ agent: this.getAgent("mls-impl-engineer"), task: this.d.context.buildDocPrompt(t, spec) })),
    ];
  ...
  /**
   * RED phase: spawn `mls-test-runner` to write failing tests for the task's acceptance criteria.
   *
  ...
  /**
   * GREEN phase: spawn `mls-impl-engineer` to make the failing tests pass.
   *
  ...
  private async implementTask(
    ...
    this.updateTask(task.id, { status: "in-progress" });
    return this.spawn("mls-impl-engineer", this.d.context.buildImplFromTestsPrompt(task, spec, testSpec, designOutput), task);
  }
   ...
   * 1. `runReview` — spawns `mls-code-reviewer`.
  ...
  /**
   * Spawn `mls-code-reviewer` for one review pass.
   *
  ...
  private async runReview(impl: string, contextOrSpec: string, task?: TaskState, deletionCheck?: DeletionCheckResult): Promise<string> {
    ...
    if (task) {
      return this.spawn("mls-code-reviewer", this.d.context.buildReviewPrompt(impl, task, contextOrSpec) + deletionWarning, task);
    }

    return this.spawn("mls-code-reviewer", this.d.context.buildReviewPromptSimple(impl, contextOrSpec) + deletionWarning);
  }
   ...
   *
   * Flow:
   * 1. Spawn `mls-impl-engineer` with the review-fix prompt.
   * 2. Run tests. If they pass, return the new implementation.
   * 3. If tests still fail, spawn a second `mls-impl-engineer` with the test-fix prompt
  ...
  private async createCheckpoint(taskId: string): Promise<string | null> {
    ...
    const label = `mls-checkpoint-${taskId}`;
   ...
   * 2. Spawn `mls-impl-engineer` with test-fix prompt. Retest.
   * 3. If still failing, spawn `mls-test-runner` as the final arbiter.
   ...
   *
   * Loops up to `maxTestRetries` times: run tests, on failure spawn `mls-impl-engineer`
   * with the test-fix prompt and update the task output. After exhausting retries, makes
   * one final attempt via a full `mls-test-runner` agent invocation. Escalates the task
  ...
  private async ensureTestsPassForTask(impl: string, task: TaskState): Promise<string> {
    ...
    const agentOutput = await this.spawn("mls-test-runner", this.d.context.buildTestPrompt(impl, task));
    if (!this.d.gates.testsPass(agentOutput)) {
      this.updateTask(task.id, { status: "escalated" });
