/**
 * MLS Pi Extension — Sprint State Management
 *
 ...
 *
 * Primary persistence is SQLite (via {@link MlsDatabase}), not this class.
...
export class StateManager {
  ...
  /**
   * Store the full specification text produced by the `mls-spec-writer` agent in Phase 1.
  ...
  /**
   * Build a human-readable multi-line status string for diagnostics (e.g., for `/mls-status`).
   ...
   * identified yet. State persistence is handled by SQLite ({@link MlsDatabase}) instead.
  ...
  private persist(): void {
    ...
    // this.deps.appendEntry("mls-state", this.state);
  }

  /**
   * Push the current state to the pi TUI.
   *
   * Sets the `"mls"` status line to `"MLS: <phase> (N/M tasks)"` while running;
   * clears it when `phase` is `"idle"` or `"complete"`.
   *
   * The `"mls-tasks"` widget is always cleared — task display is handled by the
   * `mls-live` widget in index.ts with richer formatting.
   */
  private updateUI(): void {
    ...
    if (s.phase === "idle" || s.phase === "complete") {
      this.deps.setStatus("mls", undefined);
    } else {
      const taskProgress =
        s.tasks.length > 0
          ? ` (${s.tasks.filter((t) => t.status === "complete").length}/${s.tasks.length})`
          : "";
      this.deps.setStatus("mls", `MLS: ${phaseLabel}${taskProgress}`);
    }

    // Task widget — disabled; the mls-live widget in index.ts handles task display
    // with richer formatting (agent badges, progress, collapsing).
    this.deps.setWidget("mls-tasks", undefined);
  }
}
