...
Get MLS running and build your first feature in under 5 minutes.

## 1. Install Pi

MLS is a Pi extension. First, install [Pi](https://pi.dev):
...
The MLS extension is in the `my-little-scrum-team` repository:
...
MLS is located at: `packages/mls-pi/`

## 3. Install MLS Dependencies
...
ln -s /path/to/my-little-scrum-team-pi/.pi/extensions/mls .pi/extensions/mls
```
...
```
Available commands:
  /build                         Run a full MLS sprint
  /mls-status                    Show current sprint status
```

If you don't see these, check:
- Pi is up to date: `pi --version`
- Extension symlink is valid: `ls ~/.pi/agent/extensions/mls`
...
1. **Phase 0** — MLS classifies this as a `feature` (takes ~3s)
...
The build writes state to `.mls/`:

```bash
# See the SQL database
sqlite3 .mls/mls.db "SELECT * FROM sprints ORDER BY created_at DESC LIMIT 1;"

# Watch the JSONL session log
tail -f .mls/sessions/*.jsonl | jq .
...
- Check extension installed: `ls ~/.pi/agent/extensions/mls/`
- Restart pi session
...
- Increase spawn delay in `.mls/config.json`
