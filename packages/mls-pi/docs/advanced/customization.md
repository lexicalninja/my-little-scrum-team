...
MLS agents are defined as markdown files in the extension's `agents/` directory. There is no per-project `.mls/agents/` override — all agents are loaded from the extension installation.

## Agent File Location

When MLS is installed via this plugin, the agent files live at:
...
The `mls-impl-engineer` agent explicitly reads this file before writing code.
