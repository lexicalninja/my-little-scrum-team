...
MLS ships 7 specialized agents. Each is defined by a markdown file in `packages/mls-pi/agents/`. The orchestrator selects the right agent(s) for each phase of a sprint.
...
**When it runs:**
- Phase 2 of every sprint — after `mls-spec-writer` produces the spec
...
**When it runs:**
- Before `mls-impl-engineer` for each implementation task
- After `mls-impl-engineer` to validate GREEN (all tests pass)
...
**When it runs:**
- After `mls-test-runner` has written failing tests for a task
- After `mls-code-reviewer` rejects and lists required fixes
...
**When it runs:**
- After `mls-test-runner` confirms GREEN for a task
...
1. **Phase 1 — Specification** → `mls-spec-writer` produces the spec
2. **Phase 2 — Task Breakdown** → `mls-scrum-master` produces the task list
3. **Phase 3 — Implementation** (per task, in TDD loop):
   - `mls-designer` runs first for design tasks
   - `mls-infra-engineer` runs first for infrastructure tasks
   - `mls-test-runner` writes failing tests (RED)
   - `mls-impl-engineer` makes tests pass (GREEN)
   - `mls-code-reviewer` approves or rejects; loop repeats up to `executionProfile.maxReviewIterations` times
