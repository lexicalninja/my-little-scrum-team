...
## Can I run MLS on a private codebase?

Yes. Sprint data (specs, tasks, code reviews) is stored locally in `.mls/mls.db` and never leaves your machine.
...
## Can I use MLS with other pi extensions?

Yes. MLS is just another extension. Other extensions won't interfere.
...
Resume uses the sprint state stored locally in `.mls/mls.db` and continues from the existing sprint when recovery is possible.

Current limitations:
- **Manual resume:** MLS does not auto-resume on restart; you must run `/build --resume`
- **Existing sprint only:** Resume works only when there is recoverable sprint state in `.mls/mls.db`
...
## What languages does MLS support?
...
MLS doesn't care about language. It's just coordinating agents who use read/write/bash.
...
## Can I use MLS in CI/CD pipelines?

No. MLS is designed for interactive development inside pi. Run `/build` locally and commit the results.

## Does MLS work offline?

No. MLS needs network access to:
...
## What's the difference between MLS and just using pi?
...
MLS is best for **autonomous batch development**. Pi is best for **interactive pair programming**.
...
## Does MLS generate comments or documentation?

By default, no. But you can:
- Add a skill that instructs agents to write comments
- Or use mls-designer for documentation specs
...
- `.mls/sessions/*.jsonl` log file (attach or paste tail)

## Where's the source code?

GitHub: https://github.com/lexicalninja/my-little-scrum-team

Extension is in `packages/mls-pi/`. MIT licensed.
