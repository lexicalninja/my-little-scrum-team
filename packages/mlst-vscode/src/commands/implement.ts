...
export async function handleImplement(
    ...
    if (!taskContext && !userInput) {
        stream.markdown('No task documents found in `tasks/` and no task provided.\n\nRun `@mlst /tasks` first to create a task breakdown, or describe the task directly in this message.\n');
        return;
    }
...
<!-- END COMMIT_MESSAGE -->`;
    ...
    stream.markdown(`\n\n---\n\n**Next steps:**\n- Run \`@mlst /test\` to validate with tests\n- Run \`@mlst /review\` to get code review feedback\n`);
}
