/**
 * MLST Pi Extension — Orchestrator Helpers
 *
...
import type { AgentResult, ClarificationRequest, MlstAgentConfig, TaskState, TaskType } from "../types.js";
 ...
 *
 * Mapping:
 * - `"Design"` → `"mlst-designer"`
 * - `"Infrastructure"` / `"Deployment"` → `"mlst-infra-engineer"`
 * - `"Testing"` → `"mlst-test-runner"`
 * - Everything else (`"Implementation"`, `"Documentation"`) → `"mlst-impl-engineer"`
 */
export function mapTypeToAgent(type: string): string {
  switch (type.toLowerCase()) {
    case "design": return "mlst-designer";
    case "infrastructure": case "deployment": return "mlst-infra-engineer";
    case "testing": return "mlst-test-runner";
    default: return "mlst-impl-engineer";
  }
}
