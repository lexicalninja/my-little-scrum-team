...
const AGENT_MODEL_ROLES: Record<string, AgentModelRole> = {
  "mlst-impl-engineer": "coding",
  "mlst-infra-engineer": "coding",
  "mlst-designer": "coding",
  "mlst-spec-writer": "planning",
  "mlst-scrum-master": "scrumMaster",
  "mlst-code-reviewer": "review",
  "mlst-test-runner": "tests",
};
...
/**
 * Load MLST configuration by merging global (`~/.mlst/config.json`) and
 * project-local (`<cwd>/.mlst/config.json`) settings. Project values
...
export function loadProjectConfig(cwd: string, homeDir?: string): ProjectConfig {
  ...
  const globalConfig = loadConfigFile(path.join(home, ".mlst", "config.json"));
  const projectConfig = loadConfigFile(path.join(cwd, ".mlst", "config.json"));
