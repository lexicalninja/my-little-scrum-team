/**
 * MLST Pi Extension — Execution Profiles
 *
 ...
 *   2. `"mode": "local"` key in `.mlst/config.json` — returns the local profile.
 *   3. `"executionProfile": { ... }` object in `.mlst/config.json` — merges the object over
 ...
 *
 * @param cwd - Project working directory; `.mlst/config.json` is resolved relative to this.
