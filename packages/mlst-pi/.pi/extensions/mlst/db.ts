/**
 * MLST Pi Extension — SQLite Database
 *
...
/**
 * SQLite-backed persistence layer for MLST sprint and task tracking.
 *
 ...
 */
export class MlstDatabase {
  private db: any; // better-sqlite3 Database instance

  /**
   * Open (or create) the MLST database at `<cwd>/.mlst/mlst.db`.
   *
   * Creates the `.mlst/` directory if it does not exist. Enables WAL journal mode
   ...
   *
   * @param cwd - Project working directory; the database is placed at `<cwd>/.mlst/mlst.db`.
  ...
  constructor(cwd: string) {
    ...
    if (!fs.existsSync(mlstDir)) fs.mkdirSync(mlstDir, { recursive: true });

    const dbPath = path.join(mlstDir, "mlst.db");
    let Database: any;
    try {
      ...
      throw new Error(
        `MLST requires the optional dependency "better-sqlite3". Run "npm install" in plugins/my-little-scrum-team-pi before loading the extension. Original error: ${message}`,
      );
    }
  ...
  updateSprint(id: number, updates: Partial<Pick<Sprint, "status" | "classification" | "specification" | "completed_at" | "execution_profile" | "gate_annotations" | "sprint_context" | "model" | "abort_reason">>): void {
    ...
    for (const [key, value] of Object.entries(updates)) {
      if (value !== undefined && MlstDatabase.SPRINT_COLUMNS.has(key)) {
        sets.push(`${key} = ?`);
        values.push(value);
      }
    }
  ...
  updateIssue(id: number, updates: Partial<Pick<Issue, "status" | "output" | "review_output" | "design_output" | "iteration_count" | "closed_at">>): void {
    ...
    for (const [key, value] of Object.entries(updates)) {
      if (value !== undefined && MlstDatabase.ISSUE_COLUMNS.has(key)) {
        sets.push(`${key} = ?`);
        values.push(value);
      }
    }
