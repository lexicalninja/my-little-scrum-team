/**
 * MLST Pi Extension — Direct LLM Calls
 *
 ...
 * prevent the MLST extension from loading in the subprocess (re-entrancy guard).
 *
...
export class LlmClient {
  ...
  async call(
    ...
    const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mlst-llm-"));
 ...
 *
 * Key flags:
 * - `--no-extensions`         Prevents MLST (and all other extensions) from loading in the
