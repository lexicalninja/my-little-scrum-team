...
Complete reference for `.mlst/config.json` structure.

Two config files are loaded and merged: `~/.mlst/config.json` (global) and `<project>/.mlst/config.json` (project). Project values override global values.
...
| `coding` | `mlst-impl-engineer`, `mlst-infra-engineer`, `mlst-designer` |
| `planning` | `mlst-spec-writer` |
| `scrumMaster` | `mlst-scrum-master` |
| `review` | `mlst-code-reviewer` |
| `tests` | `mlst-test-runner` |
...
MLST logs every event to a per-session JSONL file at `.mlst/sessions/<sessionId>.jsonl`. One JSON object per line.
...
### `agent_start`
```json
{ "type": "agent_start", "timestamp": 1712577600000, "agent": "mlst-spec-writer", "prompt": "...", "taskLabel": "TASK-001" }
```
...
### `agent_progress`
```json
{ "type": "agent_progress", "timestamp": 1712577600000, "agent": "mlst-impl-engineer", "taskLabel": "TASK-001", "text": "Writing auth module...", "toolCount": 5 }
```
...
```bash
ls -t .mlst/sessions/*.jsonl | head -1 | xargs cat | jq .
```

### Filter by event type
```bash
cat .mlst/sessions/*.jsonl | jq 'select(.type=="agent_end")'
```

### Calculate total cost
```bash
cat .mlst/sessions/*.jsonl | jq 'select(.type=="agent_end") | .usage.cost' | paste -sd+ | bc
```

### View timeline
```bash
cat .mlst/sessions/*.jsonl | jq '"\(.timestamp): \(.type)"'
```
...
MLST stores sprint history and task tracking in `.mlst/mlst.db` (SQLite).
...
See the [FAQ](./faq.md) for common questions, or the [Commands](./commands.md) reference for `/build` and `/mlst-status` syntax.
