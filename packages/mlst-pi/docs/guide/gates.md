...
MLST has automatic quality checkpoints that validate the build before proceeding.
...
- Or edit the spec in `.mlst/mlst.db` and retry
...
```bash
cat .mlst/sessions/*.jsonl | jq 'select(.type == "gate")'
```
