...
Learn how to set up MLST with your preferred LLM provider and customize behavior.

## Setting Up Your Provider

MLST uses models configured in **pi** (the parent CLI). You don't configure providers in MLST directly—instead, you:

1. **Configure your provider in pi** (OpenAI, Anthropic, Google, local, etc.)
2. **Choose which model to use** in `.mlst/config.json` using the `"provider/id"` format
...
Once pi is configured, you can override which model MLST uses in `.mlst/config.json`.
...
Use these exact strings in your MLST config.
...
MLST will use this model by default unless you override it in `.mlst/config.json`.

## Configuration Levels

MLST supports configuration at three levels, with lower levels overriding higher levels:

### 1. User Level (Home Directory)

Global settings for all projects on your machine.

**Location:** `~/.mlst/config.json`
...
Environment variables in pi are handled by pi itself, not MLST.
...
1. Pi's active model (from pi's configuration)
   ↓
2. ~/.mlst/config.json (user level)
   ↓
3. .mlst/config.json (project level)
```

**Example:** If pi has `openai/gpt-4o` active, but you set `"default": "openai/gpt-4-turbo"` in ~/.mlst/config.json, and then `"coding": "anthropic/claude-opus-4-5"` in .mlst/config.json:
...
Create `.mlst/config.json` in your project root:
...
```json
{
  "models": {
    ...
    "tests": "openai/gpt-4o",
    "agents": {
      "mlst-spec-writer": "openai/gpt-4o",
      "mlst-scrum-master": "openai/gpt-4o",
      "mlst-impl-engineer": "openai/gpt-4o"
    }
  },
  "executionProfile": {
    "name": "balanced",
...
MLST uses the model system from **pi** (the parent CLI). Models are specified as `"provider/id"` strings.
...
Models are configured in pi's configuration, not in `.mlst/config.json`. If you don't specify a model in MLST config, it uses whatever model is currently active in pi.
...
- Keys: Agent names (e.g., `"mlst-spec-writer"`, `"mlst-impl-engineer"`)
...
```json
{
  "models": {
    ...
    "agents": {
      "mlst-spec-writer": "anthropic/claude-opus-4-5",
      "mlst-test-runner": "openai/gpt-4-turbo"
    }
  }
}
```
...
Configure how MLST orchestrates agents and handles iterations. This is a nested object inside the main config.
...
MLST reads your configuration on startup, but it does not comprehensively validate every field.

If something is not working as expected, manually check that:
- `.mlst/config.json` contains valid JSON
...
✅ Set up `.mlst/config.json` with your preferred models  
