...
import { Orchestrator, type OrchestratorDeps } from "../.pi/extensions/mlst/orchestrator/index.js";
import type { AgentResult, MlstEvent, TaskState } from "../.pi/extensions/mlst/types.js";

// Mock spawnAgent & friends
vi.mock("../.pi/extensions/mlst/agents.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../.pi/extensions/mlst/agents.js")>();
  return {
    ...actual,
...
function makeTask(overrides: Partial<TaskState> = {}): TaskState {
  return {
    ...
    assignedAgent: "mlst-impl-engineer",
    iterationCount: 0,
    ...overrides,
  };
}
...
function buildDeps(overrides: Partial<OrchestratorDeps> = {}): OrchestratorDeps {
  ...
  const defaults: OrchestratorDeps = {
    ...
    agents: [
      { name: "mlst-spec-writer", description: "", systemPrompt: "", filePath: "/agents/mlst-spec-writer.md" },
      { name: "mlst-scrum-master", description: "", systemPrompt: "", filePath: "/agents/mlst-scrum-master.md" },
      { name: "mlst-impl-engineer", description: "", systemPrompt: "", filePath: "/agents/mlst-impl-engineer.md" },
      { name: "mlst-test-runner", description: "", systemPrompt: "", filePath: "/agents/mlst-test-runner.md" },
      { name: "mlst-code-reviewer", description: "", systemPrompt: "", filePath: "/agents/mlst-code-reviewer.md" },
      { name: "mlst-designer", description: "", systemPrompt: "", filePath: "/agents/mlst-designer.md" },
      { name: "mlst-infra-engineer", description: "", systemPrompt: "", filePath: "/agents/mlst-infra-engineer.md" },
    ],
    profile: {
      name: "test",
      group1Concurrency: 1,
...
function getEmittedEvents(deps: OrchestratorDeps): MlstEvent[] {
...
describe("executeImplTask escalation retry path", () => {
  it("resets iteration count to zero and re-runs the task on retry", async () => {
    ...
    const { spawnAgent } = await import("../.pi/extensions/mlst/agents.js");
    let spawnCount = 0;
...
describe("executeImplTask escalation abort path", () => {
  it("sets controller signal aborted and captures abort reason with task label", async () => {
    ...
    const endEvent = sprintEndEvents[sprintEndEvents.length - 1] as Extract<MlstEvent, { type: "sprint_end" }>;
...
describe("phase4 abort report event fields", () => {
  it("emits sprint_end with aborted, abortReason, summary, and timestamp fields", async () => {
    ...
    const endEvent = sprintEndEvents[0] as Extract<MlstEvent, { type: "sprint_end" }>;
