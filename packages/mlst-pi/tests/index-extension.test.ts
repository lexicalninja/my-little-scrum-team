/**
 * Tests for the mlstExtension default export — command handlers and pi event hooks.
 *
 * Strategy: call mlstExtension(fakePi) with a fake ExtensionAPI that captures
 * registered handlers/listeners, then invoke them directly with a fake ctx.
 * All heavy dependencies (Orchestrator, Dashboard, MlstDatabase, etc.) are mocked.
 *
...
vi.mock("../.pi/extensions/mlst/db.js", () => ({
  MlstDatabase: class { constructor() { return mockDbInstance; } },
}));
...
vi.mock("../.pi/extensions/mlst/config.js", () => ({
...
describe("/mlst-status — handleStatus", () => {
  let pi: CapturedPi;
  beforeEach(() => {
    vi.clearAllMocks();
    mockStateInstance.getState.mockReturnValue({ phase: "idle", tasks: [] });
  ...
  it("notifies 'No active sprint.' when no sprint exists", async () => {
    mockDbInstance.getActiveSprint.mockReturnValue(null);
    const ctx = makeCtx();
    await pi.commands["mlst-status"]("", ctx);
