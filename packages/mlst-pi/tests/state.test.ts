...
import { StateManager, type StateManagerDeps } from "../.pi/extensions/mlst/state.js";
import type { TaskState, TaskStatus } from "../.pi/extensions/mlst/types.js";
...
function makeTask(overrides: Partial<TaskState> = {}): TaskState {
  return {
    ...
    acceptanceCriteria: ["criterion"], filesAffected: [], assignedAgent: "mlst-impl",
...
describe("StateManager", () => {
  ...
  describe("setPhase", () => {
    ...
    it("calls setStatus with the correct label", () => {
      sm.setPhase("phase1");
      expect(deps.setStatus).toHaveBeenCalledWith("mlst", "MLST: Phase 1: Specification");
    });
    ...
    it("includes task progress in status when tasks exist", () => {
      ...
      sm.setPhase("phase3");
      expect(deps.setStatus).toHaveBeenCalledWith("mlst", "MLST: Phase 3: Execution (1/2)");
    });
    ...
    it("clears status for idle phase", () => {
      ...
      vi.mocked(deps.setStatus).mockClear();
      sm.setPhase("idle");
      expect(deps.setStatus).toHaveBeenCalledWith("mlst", undefined);
    });
    ...
    it("uses the raw phase name when no label is mapped", () => {
      ...
      expect(deps.setStatus).toHaveBeenCalledWith("mlst", "MLST: unknown-phase");
    });
  });
  ...
  describe("UI: widget", () => {
    it("clears mlst-tasks widget (task display handled by mlst-live widget)", () => {
      sm.setPhase("phase3");
      sm.setTasks([
      ...
      expect(lastCall[0]).toBe("mlst-tasks");
    ...
    it("clears widget on idle phase", () => {
      ...
      const widgetCalls = vi.mocked(deps.setWidget).mock.calls;
      const lastCall = widgetCalls[widgetCalls.length - 1];
      expect(lastCall).toEqual(["mlst-tasks", undefined]);
    });
