...
import { parseStructuredTasks, parseTaskHeadings, createTaskState } from "../.pi/extensions/mlst/orchestrator/task-parser.js";
import type { AgentResult, TaskState } from "../.pi/extensions/mlst/types.js";
...
function buildDeps(overrides: Partial<OrchestratorDeps> = {}): OrchestratorDeps {
  ...
  const defaults: OrchestratorDeps = {
    ...
    agents: [
      makeAgent("mlst-impl-engineer"),
      makeAgent("mlst-spec-writer"),
      makeAgent("mlst-scrum-master"),
      makeAgent("mlst-code-reviewer"),
      makeAgent("mlst-test-runner"),
      makeAgent("mlst-designer"),
      makeAgent("mlst-infra-engineer"),
    ],
    profile: {
      name: "test",
      group1Concurrency: 1,
...
describe("Orchestrator task parsing", () => {
  ...
  it("createTaskState maps type to correct agent", () => {
    ...
    expect(task.assignedAgent).toBe("mlst-designer");
  ...
  it("createTaskState defaults to 'Implementation' type and impl agent", () => {
    ...
    expect(task.assignedAgent).toBe("mlst-impl-engineer");
  });
...
describe("Orchestrator.scaffold()", () => {
  ...
  it("spawns mlst-impl-engineer when project is empty", async () => {
    const deps = buildDeps();
...
describe("Orchestrator.runTests() framework detection", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mlst-run-tests-"));
  });
...
describe("Orchestrator.runLint()", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mlst-lint-"));
  });
