...
import type { AgentResult } from "../.pi/extensions/mlst/types.js";
...
describe("Orchestrator.run() PRD shortcut", () => {
  ...
  it("uses PRD content directly as spec (skips spec-writer agent)", async () => {
    ...
    expect(agentCalls).not.toContain("mlst-spec-writer");
  });
});
...
describe("Orchestrator private getAgent()", () => {
  it("returns the agent config when found", () => {
    const deps = buildDeps();
    const orchestrator = new Orchestrator(deps);
    const agent = (orchestrator as any).getAgent("mlst-impl-engineer");
    expect(agent.name).toBe("mlst-impl-engineer");
  });
