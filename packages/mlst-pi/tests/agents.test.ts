...
describe("loadAgents", () => {
  it("parses a valid agent with all fields", () => {
    ...
    const agent = agents.find((a) => a.name === "mlst-test-agent");
  ...
  it("applies config-driven model overrides when provided", () => {
    const agents = loadAgents(fixturesDir, {
      ...
      models: {
        agents: {
          "mlst-test-agent": "openai/o3-mini",
        },
      },
    });
...
describe("loadProviderProfile", () => {
  ...
  it("uses per-project config override from .mlst/config.json", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mlst-test-"));
    try {
