...
import type { AgentResult, GatePoint, TaskState } from "../.pi/extensions/mlst/types.js";
...
describe("ExecutionProfile humanGates", () => {
  it("CLOUD_PROFILE defaults to no gates (opt-in)", async () => {
    const { CLOUD_PROFILE } = await import("../.pi/extensions/mlst/execution-profiles.js");
    expect(CLOUD_PROFILE.humanGates).toEqual([]);
  });

  it("LOCAL_PROFILE defaults to no gates (fully autonomous)", async () => {
    const { LOCAL_PROFILE } = await import("../.pi/extensions/mlst/execution-profiles.js");
...
describe("types", () => {
  it("exports GatePoint, HumanGateResult, EscalationAction, PipelineMode types", async () => {
    ...
    const types = await import("../.pi/extensions/mlst/types.js");
...
describe("fullPipeline — review-only mode", () => {
  it("stops after Phase 2 when pipelineMode is review-only", async () => {
    const deps = buildDeps({
      ...
      agents: [
        {
          name: "mlst-spec-writer",
          description: "",
          systemPrompt: "",
          filePath: "/agents/mlst-spec-writer.md",
        },
        {
          name: "mlst-scrum-master",
          description: "",
          systemPrompt: "",
          filePath: "/agents/mlst-scrum-master.md",
        },
      ],
    });
...
describe("database — resume columns", () => {
  it("Sprint type includes new resume columns", async () => {
    const { MlstDatabase } = await import("../.pi/extensions/mlst/db.js");
    // Verify the module imports cleanly with new types
    expect(MlstDatabase).toBeDefined();
  });
});
