...
describe("Suite health — full test suite verification", () => {
  ...
  it(".github/workflows/mlst-pi-tests.yml exists and is syntactically valid YAML", () => {
    ...
    expect(
      ...
      "mlst-pi-tests.yml should exist",
    ).toBe(true);
    ...
    expect(content).toContain("packages/mlst-pi");
    // Must have npm test step
    expect(content).toContain("npm test");
  });

  it("no orphaned mlst temp directories created by this test run", () => {
    // Check for mlst-test temp dirs created in the last 60 seconds
    ...
    const recentOrphanedDirs = entries.filter((entry) => {
      if (!mlstPrefixes.some((prefix) => entry.startsWith(prefix))) {
        return false;
      }
