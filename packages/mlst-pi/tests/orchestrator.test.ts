...
import { Orchestrator, extractClarifications, type OrchestratorDeps } from "../.pi/extensions/mlst/orchestrator/index.js";
...
describe("OrchestratorDeps", () => {
  it("is publicly exported from orchestrator module", async () => {
    ...
    const mod = await import("../.pi/extensions/mlst/orchestrator/index.js");
...
describe("extractClarifications()", () => {
  it("returns null for output with no clarification markers", () => {
    ...
    expect(extractClarifications(output, "mlst-impl-engineer")).toBeNull();
  });
...
Continuing with 404 for now.`;
    ...
    const result = extractClarifications(output, "mlst-impl-engineer", "TASK-001");
    expect(result).not.toBeNull();
    expect(result!.agent).toBe("mlst-impl-engineer");
...
Proceeding with REST and no auth for now.`;
    ...
    const result = extractClarifications(output, "mlst-designer");
    expect(result).not.toBeNull();
  ...
  it("handles markers without the taskLabel", () => {
    ...
    const result = extractClarifications(output, "mlst-spec-writer");
    expect(result).not.toBeNull();
    expect(result!.taskLabel).toBeUndefined();
    expect(result!.agent).toBe("mlst-spec-writer");
  });
});
        ...
        agents: [
          {
            ...
            systemPrompt: "",
            filePath: "/agents/mlst-impl-engineer.md",
          },
        ],
      });
        ...
        agents: [
          {
            ...
            systemPrompt: "",
            filePath: "/agents/mlst-designer.md",
          },
        ],
      });
      const orchestrator = new Orchestrator(deps);
