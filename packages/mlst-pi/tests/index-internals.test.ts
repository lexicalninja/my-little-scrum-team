...
import { __test__ } from "../.pi/extensions/mlst/index.js";

// ─── Helpers ──────────────────────────────────────────────────────────────────

function ts() { return Date.now(); }

function agentEndEvent(agent = "mlst-impl-engineer", taskLabel = "TASK-001", usage = { input: 100, output: 200, cost: 0.05 }) {
...
describe("applyWidgetEvent — phase events", () => {
  ...
  it("clears agents map on each phase transition", () => {
    ...
    w.agents.set("mlst-designer:test", { agent: "mlst-designer", taskLabel: "test", status: "running", progress: "", toolCount: 0, startTime: ts() });
    __test__.applyWidgetEvent(w, { type: "phase", phase: "phase1", timestamp: ts() });
...
describe("applyWidgetEvent — agent_start", () => {
  it("adds a running agent slot with the given key", () => {
    const w = __test__.createWidgetState();
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-designer", taskLabel: "TASK-001", prompt: "", timestamp: ts() });
    const slot = w.agents.get("mlst-designer:TASK-001");
    expect(slot).toBeDefined();
    expect(slot.status).toBe("running");
    expect(slot.agent).toBe("mlst-designer");
  ...
  it("falls back to activePipelineId when taskLabel is empty", () => {
    ...
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-spec-writer", taskLabel: "", prompt: "", timestamp: ts() });
    const slot = w.agents.get("mlst-spec-writer:");
...
describe("applyWidgetEvent — agent_end", () => {
  it("marks the matching slot as done", () => {
    const w = __test__.createWidgetState();
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-impl-engineer", taskLabel: "TASK-001", prompt: "", timestamp: ts() });
    __test__.applyWidgetEvent(w, agentEndEvent("mlst-impl-engineer", "TASK-001"));
    const slot = w.agents.get("mlst-impl-engineer:TASK-001");
  ...
  it("records tokens and cost on the slot", () => {
    const w = __test__.createWidgetState();
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-impl-engineer", taskLabel: "TASK-001", prompt: "", timestamp: ts() });
    __test__.applyWidgetEvent(w, agentEndEvent("mlst-impl-engineer", "TASK-001", { input: 500, output: 250, cost: 0.1 }));
    const slot = w.agents.get("mlst-impl-engineer:TASK-001");
  ...
  it("attributes tokens to matching task", () => {
    ...
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-impl-engineer", taskLabel: "TASK-001", prompt: "", timestamp: ts() });
    __test__.applyWidgetEvent(w, agentEndEvent("mlst-impl-engineer", "TASK-001", { input: 100, output: 100, cost: 0.02 }));
  ...
  it("attributes tokens to pipeline step when no matching task", () => {
    const w = __test__.createWidgetState();
    __test__.applyWidgetEvent(w, { type: "phase", phase: "phase1", timestamp: ts() });
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-spec-writer", taskLabel: "phase1", prompt: "", timestamp: ts() });
    __test__.applyWidgetEvent(w, agentEndEvent("mlst-spec-writer", "phase1", { input: 300, output: 300, cost: 0.06 }));
  ...
  it("is a no-op when the slot is not found", () => {
    ...
    expect(() => __test__.applyWidgetEvent(w, agentEndEvent("mlst-impl-engineer", "TASK-999"))).not.toThrow();
  });

  it("handles missing usage gracefully (defaults to 0)", () => {
    const w = __test__.createWidgetState();
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-impl-engineer", taskLabel: "T", prompt: "", timestamp: ts() });
    // usage fields are undefined
    __test__.applyWidgetEvent(w, { type: "agent_end", agent: "mlst-impl-engineer", taskLabel: "T", output: "", usage: {} as any, timestamp: ts() });
    const slot = w.agents.get("mlst-impl-engineer:T");
...
describe("applyWidgetEvent — agent_progress", () => {
  it("updates progress text and toolCount on matching slot", () => {
    const w = __test__.createWidgetState();
    __test__.applyWidgetEvent(w, { type: "agent_start", agent: "mlst-test-runner", taskLabel: "TASK-002", prompt: "", timestamp: ts() });
    __test__.applyWidgetEvent(w, { type: "agent_progress", agent: "mlst-test-runner", taskLabel: "TASK-002", text: "Running tests…", toolCount: 3, timestamp: ts() });
    const slot = w.agents.get("mlst-test-runner:TASK-002");
  ...
  it("is a no-op when slot is not found", () => {
    const w = __test__.createWidgetState();
    expect(() => __test__.applyWidgetEvent(w, { type: "agent_progress", agent: "mlst-reviewer", taskLabel: "TASK-X", text: "hi", toolCount: 1, timestamp: ts() })).not.toThrow();
  });
});
...
describe("applyWidgetEvent — sprint_end", () => {
  it("sets phase to Complete and clears running agents", () => {
    const w = __test__.createWidgetState();
    w.agents.set("mlst-designer:T", { agent: "mlst-designer", taskLabel: "T", status: "running", progress: "", toolCount: 0, startTime: ts() });
...
describe("applyFooterEvent", () => {
  it("creates a new agent total entry on the first agent_end", () => {
    ...
    __test__.applyFooterEvent(footer, agentEndEvent("mlst-designer", "T", { input: 100, output: 200, cost: 0.03 }));
    const total = footer.agentTotals.get("mlst-designer");
  ...
  it("accumulates into an existing entry on subsequent agent_end events", () => {
    const footer = __test__.createFooterState();
    __test__.applyFooterEvent(footer, agentEndEvent("mlst-designer", "T1", { input: 100, output: 100, cost: 0.02 }));
    __test__.applyFooterEvent(footer, agentEndEvent("mlst-designer", "T2", { input: 200, output: 200, cost: 0.04 }));
    const total = footer.agentTotals.get("mlst-designer");
  ...
  it("tracks multiple agents independently", () => {
    const footer = __test__.createFooterState();
    __test__.applyFooterEvent(footer, agentEndEvent("mlst-designer", "T1", { input: 100, output: 100, cost: 0.02 }));
    __test__.applyFooterEvent(footer, agentEndEvent("mlst-impl-engineer", "T2", { input: 200, output: 200, cost: 0.04 }));
  ...
  it("handles missing usage fields (defaults to 0)", () => {
    const footer = __test__.createFooterState();
    __test__.applyFooterEvent(footer, { type: "agent_end", agent: "mlst-reviewer", taskLabel: "T", output: "", usage: {} as any, timestamp: ts() });
    const total = footer.agentTotals.get("mlst-reviewer");
...
describe("agentStyle", () => {
  it("returns known color/initial for recognised agent names", () => {
    const s = __test__.agentStyle("mlst-designer");
  ...
  it("returns correct style for mlst-impl-engineer", () => {
    const s = __test__.agentStyle("mlst-impl-engineer");
...
describe("shortAgent", () => {
  it("strips the mlst- prefix and uppercases", () => {
    expect(__test__.shortAgent("mlst-designer")).toBe("DESIGNER");
  });

  it("replaces hyphens with spaces", () => {
    expect(__test__.shortAgent("mlst-impl-engineer")).toBe("IMPL ENGINEER");
  });

  it("handles names without mlst- prefix (hyphens become spaces)", () => {
