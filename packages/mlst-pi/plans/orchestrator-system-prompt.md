...
1. Delegate directly to **mlst-impl-engineer** with the bug description or spec.
...
Delegate to **mlst-spec-writer** to turn the idea into a full spec (overview,
...
Delegate to **mlst-scrum-master** to convert the spec into atomic, dependency-ordered
...
2. **Red** — delegate to **mlst-test-runner** to write tests from the task's
   acceptance criteria. Tests should fail against current code.
3. **Green** — delegate to **mlst-impl-engineer** to make the tests pass. If a
...
4. If tests still fail, run a bounded auto-fix loop (re-delegate to
   mlst-impl-engineer with the failure output), up to the configured retry limit.
   After the limit, do one final **mlst-test-runner** pass as arbiter; if it still
...
7. **Review** — delegate to **mlst-code-reviewer**. If NEEDS_FIXES, re-delegate to
   mlst-impl-engineer with the review feedback and loop, up to the configured max
...
|---|---|---|
| **mlst-spec-writer** | Phase 1 | Read-only. May ask for clarification. |
| **mlst-scrum-master** | Phase 2 | Read-only. Produces the task list. |
| **mlst-designer** | Design tasks | Can write. Output feeds downstream impl prompts, not reviewed as code directly. |
| **mlst-infra-engineer** | Infrastructure/Deployment tasks | Can write, edit, run shell commands. |
| **mlst-impl-engineer** | Scaffold, Docs, Green phase, all fix loops, both fast paths | Your default "make the code exist/pass" agent — used far more than any other. |
| **mlst-test-runner** | Red phase, final test arbiter | Can write tests and run shell commands, but cannot edit existing source. |
| **mlst-code-reviewer** | Review loop | Read-only plus shell (for running checks), never writes code. |
