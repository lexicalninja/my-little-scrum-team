...
You describe what you want built. MLS classifies your input, then routes it through a pipeline of AI agents:
...
MLS classifies your input and picks the fastest path:
...
cd my-little-scrum-team/packages/mls-app
npm install
npm run build
npm link  # makes `mls` available globally
```
...
runtime relative to the installed location. Set `MLS_SKILLS_DIR` to point
somewhere else:

```bash
export MLS_SKILLS_DIR=/path/to/skills
```
...
```bash
# Simple feature
mls build "Add a REST endpoint that returns user profiles"
...
mls build 'implement the feature from lexicalninja/my-little-scrum-team#25'

# From a spec file
mls build --from-file spec.md

# Non-interactive (auto-accept all prompts)
mls build --yes "Fix the login bug where sessions expire immediately"

# Resume an interrupted build
mls build --resume abc123
...
MLS can fetch a GitHub issue and use its title and body as the build description. This requires the [GitHub CLI (`gh`)](https://cli.github.com) to be installed and authenticated (`gh auth login`).

| Syntax | Description |
|---|---|
| `mls build '#25'` | Issue 25 in the current repo (inferred from `git remote origin`) |
| `mls build --from-issue 25` | Same as above via flag |
| `mls build 'owner/repo#25'` | Issue 25 in the specified repo |
| `mls build 'see #25 for context'` | Inline reference — `#25` is expanded in place |
...
Run MLS as an MCP server so Claude Code can call it as a tool. No local clone needed — `npx` pulls the package directly from GitHub Packages.
...
> **Note:** this assumes `mls` is on your `PATH` via `npm link` (see [Install from source](#install-from-source) above). If it isn't, use the absolute-path form below.
...
```json
{
  "mcpServers": {
    "mls": {
      "command": "node",
      "args": ["/absolute/path/to/mls-app/dist/bin/mls.js", "serve"],
      "env": {
        "GITHUB_TOKEN": "ghp_your_token_here"
      }
    }
...
| Tool | Description |
|---|---|
| `mls_build` | Start a new build from a description |
| `mls_status` | Check status of a build or list recent runs |
| `mls_resume` | Resume an interrupted build |
| `mls_list_runs` | List recent build runs |
| `mls_config` | View current configuration |

#### MCP Resources

Build artifacts are available as MCP resources:

- `mls://runs/{runId}/status` — Build status and task list
- `mls://runs/{runId}/specification` — Generated specification
- `mls://runs/{runId}/tasks` — Task breakdown

### Option 3: Library

```typescript
import { OrchestrationEngine, loadConfig, createGitHubModelsProvider } from '@lexicalninja/mls';
...
3. Project config (`.mls/config.json`)
4. Global config (`~/.mls/config.json`)

### Model overrides

Override which model each agent uses in `.mls/config.json`:
...
MLS is published to GitHub Packages via a GitHub Actions workflow. To release a new version:

```bash
# 1. Update version in mls-app/package.json
cd mls-app
npm version patch  # or minor, major

# 2. Tag and push
git tag "mls-app/v$(node -p 'require("./package.json").version')"
git push origin --tags
```

The workflow at `.github/workflows/publish-mls.yml` will automatically:
1. Run tests
