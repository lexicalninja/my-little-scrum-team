#!/usr/bin/env node
/**
 * MLS CLI entry point — multi-agent scrum team.
 */
...
program
  ...
  .action(async (action, key, value) => {
    ...
    if (!action || action === 'get') {
      ...
      await mkdir('.mls', { recursive: true });
      await writeFile('.mls/config.json', JSON.stringify({
        models: {},
        maxSteps: 50,
        maxReviewIterations: 5,
      }, null, 2));
      console.error(chalk.green('Created .mls/config.json'));
    } else if (action === 'set' && key && value) {
      console.error(`Set ${key} = ${value}`);
      console.error(chalk.gray('(Edit .mls/config.json directly for now)'));
    }
  });
