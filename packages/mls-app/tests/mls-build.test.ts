/**
 * Tests for bin/mls.ts — `build` command action handler.
 *
 * Tests the branching logic of the `mls build` command:
 ...
 *   - Dynamic import of bin/mls.ts triggers program.parse(), which fires the async action.
...
/** Set process.argv to simulate: `mls build <...rest>` */
function setArgv(...rest: string[]) {
  process.argv = ['node', 'mls', 'build', ...rest];
}

/**
 * Register all mocks via vi.doMock and dynamically import bin/mls.ts.
 ...
 */
async function loadMlsWithMocks() {
  // bin/mls.ts uses createRequire(import.meta.url)('../../package.json') for version.
  // When running from source (bin/mls.ts), the path resolves to root package.json
